...
def train_one_config(
    ...
    # Pull lambda_align from config, fall back to args for single-run / no_search
    ...
    align_criterion = nn.CosineEmbeddingLoss(margin=args.cos_margin)
    ...
    print(f"alpha={args.alpha}  temperature={args.temperature}  "
          f"lambda_align={lambda_align}  cos_margin={args.cos_margin}")
    ...
    for epoch in range(config["epochs"]):
        ...
        for batch in pbar:
            ...
            kd_loss = kd_loss_fn(student_logits, teacher_logits, labels,
                                 args.temperature, args.alpha, args.task)
...
def train_one_config_mnli(
    ...
    for epoch in range(config["epochs"]):
        ...
        for batch in pbar:
            ...
            kd_loss = kd_loss_fn(student_logits, teacher_logits, labels,
                                 args.temperature, args.alpha, "mnli")
...
def parse_args():
    ...
    # KD params
    p.add_argument("--alpha", type=float, default=0.5, help="Weight on KD loss vs hard labels.")
    p.add_argument("--temperature", type=float, default=3.0, help="KD temperature.")
    ...
    p.add_argument("--lambda_align", type=float, default=0.5,
                   help="Default weight on alignment loss. Overridden per-config if lambda_align is set in DEFAULT_CONFIGS.")
    p.add_argument("--cos_margin", type=float, default=0.2, help="Margin for CosineEmbeddingLoss.")
...
def main():
    ...
    print(f"Default lambda_align={args.lambda_align}  cos_margin={args.cos_margin}")
    print("=" * 96)
    ...
    out = {
        ...
        "alpha": args.alpha,
        "temperature": args.temperature,
        "default_lambda_align": args.lambda_align,
        "cos_margin": args.cos_margin,
        "aug_data_path": args.aug_data_path,
        "use_all_variants": args.use_all_variants,
